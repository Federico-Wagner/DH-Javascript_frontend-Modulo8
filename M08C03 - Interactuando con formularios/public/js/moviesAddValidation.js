...
//-------------------DE REGISTRO DE PELÍCULAS------------------//
    ...
    title.addEventListener("input",function(event){
        if(title.value.length == 0 ){
            ...
            errors.push("El titulo es invalido")
        }else{
    ...
    rating.addEventListener("input",function(event){
        if(rating.value.length == 0 || rating.value < 0 || rating.value > 10){
            ...
            errors.push("El rating es invalido")
        }else{
    ...
    awards.addEventListener("input",function(event){
        if(awards.value.length == 0 || awards.value < 0 || awards.value > 10){
            ...
            errors.push("Los premios son invalidos")
        }else{
    ...
    release_date.addEventListener("input",function(event){
        if(release_date.value.length == 0 ){
            ...
            errors.push("La fecha d elanzamiento es invalida")
        }else{
    ...
    length.addEventListener("input",function(event){
        if(length.value.length == 0 || length.value < 60 || length.value > 360){
            ...
            errors.push("La duracion es es invalida")
        }else{
    ...
    genre_id.addEventListener("input",function(event){
        if(genre_id.value.length == 0 ){
            ...
            errors.push("Genero no ingresado")
        }else{
    ...
    formulario.addEventListener("submit", function(event){
        if(!(
            title.classList.contains("is-valid") &&
            rating.classList.contains("is-valid") &&
            awards.classList.contains("is-valid") &&
            release_date.classList.contains("is-valid") &&
            length.classList.contains("is-valid") &&
            genre_id.classList.contains("is-valid"))
            ){
                //INVALID
